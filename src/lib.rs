...
mod common2018 {
    ...
    >
    {
        ...
        fn run(&self) {
            ...
            let input_path = format!("./src/solutions/year{:04}/day{:02}-input.txt", 2018, day);

            let input_full = fs::read_to_string(input_path).expect("Failed to load input.");
...
impl Problem<int, int> for ChronalCalibration {
    ...
    fn solve(&self, input: &[&str]) -> (int, int) {
        let numbers: Vec<int> = input
            ...
            .collect();
        let frequency_sum = numbers.iter().copied().sum::<int>();
